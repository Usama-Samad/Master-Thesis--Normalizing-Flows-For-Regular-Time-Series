...
#         self.register_buffer('slope', torch.exp(torch.tensor(1.)))
        
#     def shiesh_(self, x: Tensor) -> Tensor:
#         return torch.arcsinh(torch.exp(torch.tensor(1))*torch.sinh(x))
        
#     def Dshiesh(self, x: Tensor) -> Tensor:
#         return torch.exp(torch.tensor(1))*torch.cos(x)/(1+(torch.exp(torch.tensor(1))*torch.sinh(x))**2)**0.5
...
# class shiesh_inverse(nn.Module):
#     threshold: Tensor
#     slope: Tensor
...
#     def shiesh_inv(self, x: Tensor) -> Tensor:
#         return torch.arcsinh(torch.exp(-1)*torch.sinh(x))
        
#     def Dshiesh_inv(self, x: Tensor) -> Tensor:
#         return torch.exp(-1)*torch.cos(x)/(1+(torch.exp(-1)*torch.sinh(x))**2)**0.5

#     def forward(self, x: Tensor) -> (Tensor, Tensor):
#         mask = x.abs() <= self.threshold
#         x_out = torch.where(mask, self.shiesh_inv(x), x - torch.sign(x))
...
class LinearMaskedCoupling(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, n_hidden, mask, cond_label_size=None):
        ...
        self.shiesh = shiesh()
        self.snet_linear1 = nn.Linear(
                input_size + (cond_label_size if cond_label_size is not None else 0),
                hidden_size,
            )
        self.snet_linear2 = nn.Linear(hidden_size, hidden_size)
        self.snet_linear3 = nn.Linear(hidden_size, input_size)
        ###############################################   
...
#         self.t_net = copy.deepcopy(self.s_net)
    ...
    def forward(self, x, y=None):
        ...
        # run through model
        s = mx if y is None else torch.cat([y, mx], dim=-1)
        s = self.snet_linear1(s)
        for _ in range(self.n_hidden):
            s, s_jacobian = self.shiesh(s)
            s = self.snet_linear2(s)
        s, s_jacobian = self.shiesh(s)
        s = self.snet_linear3(s)
            
        
        t = mx if y is None else torch.cat([y, mx], dim=-1)
        t = self.snet_linear1(t)
        for _ in range(self.n_hidden):
            t, t_jacobian = self.shiesh(t)
            t = self.snet_linear2(t)
        t, t_jacobian = self.shiesh(t)
        t = self.snet_linear3(t)
        t = t * (1 - self.mask)
...
class MADE(nn.Module):
    def __init__(
        ...
        #elif activation == "Shiesh":
            #activation_fn = shiesh()
        else:
    ...
    def forward(self, x, y=None):
        ...
        #u, log_abs_det_jacobian = self.shiesh_act(u,inverse = False)
        return u, log_abs_det_jacobian
    ...
    def inverse(self, u, y=None, sum_log_abs_det_jacobians=None):
        ...
        log_abs_det_jacobian = loga
        #=self.shiesh_act(x,inverse = False)
        return x, log_abs_det_jacobian
